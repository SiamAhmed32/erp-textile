...
import { mockCustomerLedgerData, CustomerLedgerItem } from "../_components/types";
            ...
            <Flex className="flex-col gap-6 max-w-6xl mx-auto">
                ...
                <Flex className="items-center justify-between pb-4 border-b">
                    <Flex className="items-center gap-4">
                        ...
                        <Box>
                            <PrimaryHeading className="text-xl md:text-2xl">{customer.customerName}</PrimaryHeading>
                            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{customer.customerId}</p>
                        </Box>
                    </Flex>
                    <Button
                        size="sm"
                ...
                <Box className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <CardContent className="p-5">
                            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                                <Box>
                                    ...
                                    <p className="text-sm font-semibold text-slate-700">{customer.customerName}</p>
                                </Box>
                                <Box>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Customer ID</p>
                                    <p className="text-sm font-semibold text-slate-700">{customer.customerId}</p>
                                </Box>
                                <Box className="col-span-2">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                                        <MapPin className="h-3 w-3" /> Address
                                    </p>
                                    <p className="text-sm font-medium text-slate-600 leading-relaxed">{customer.address}</p>
                                </Box>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="shadow-xs border-slate-200">
                        ...
                        <CardContent className="p-5 flex flex-col justify-center h-full min-h-[140px]">
                            <Box>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Current Balance</p>
                                ...
                                <p className="text-[10px] font-medium text-slate-500 mt-2 italic">
                                    Total outstanding balance as of today
                                </p>
                            </Box>
                        </CardContent>
                    </Card>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {filteredTransactions.map((t) => (
                                        <tr key={t.id} className="hover:bg-slate-50/80 transition-colors group">
                                            ...
                                            <td className={`px-6 py-4 text-right text-sm font-bold ${t.isCredit ? 'text-green-600' : 'text-slate-800'}`}>
