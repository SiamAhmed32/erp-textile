...
export default function RecordReceiptModal({
  ...
  const reachableAccounts = allAccounts.filter(
    (acc) =>
      acc.type === "ASSET" &&
      (acc.name.toLowerCase().includes("receivable") ||
        acc.name.toLowerCase().includes("reachable")),
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify.success("Receipt recorded successfully");
      resetForm();
      onOpenChange(false);
    } catch (error: any) {
      ...
      notify.error(
        error?.data?.message ||
          "Could not record the receipt. Please try again.",
      );
    }
  };
        ...
        <div className="grid grid-cols-2 gap-4 bg-zinc-50/50 p-4 rounded-xl border border-zinc-100">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[9px] text-emerald-600 italic font-medium">
              * Suggest: Accounts Receivable (Editable)
            </p>
          </div>
        </div>
