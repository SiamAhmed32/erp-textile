    ...
    <CustomModal
      ...
      title={
        <div className="flex items-center gap-2 uppercase tracking-widest text-[10px] font-black text-zinc-400">
          Treasury — <span className="text-zinc-900">Record Repayment</span>
        </div>
      }
      maxWidth="500px"
    >
...
export default function LoanDetailPage({
      ...
      },
      {
        ...
        accessor: (row: LoanRepayment) => (
          <span className="text-zinc-500 font-medium text-sm">
            {fmt(Number(row.remainingBalance))}
          </span>
        ),
      },
    ],
  ...
  const stats = useMemo(() => {
    ...
    const paid =
      loan.repayments?.reduce((sum, r) => sum + Number(r.principal), 0) || 0;
    const interestPaid =
      loan.repayments?.reduce((sum, r) => sum + Number(r.interest), 0) || 0;
  ...
  if (isLoadingLoan) {
    return (
      <Container className="py-20 text-center">
        <div className="animate-pulse text-zinc-400 font-black uppercase tracking-widest">
          Securing Treasury Data...
        </div>
      </Container>
    );
  }
    ...
    return (
      <Container className="pb-10 text-center py-24">
        ...
        <h2 className="text-xl font-bold text-zinc-900">
          Stakeholder Record Missing
        </h2>
        <p className="text-zinc-500 mt-2 max-w-sm mx-auto">
          This debt facility might have been archived or the ID is incorrect in
          the treasury system.
        </p>
        <Link href="/accounting/loan-management">
          <Button
            variant="outline"
            className="mt-8 rounded-xl h-11 px-8 font-bold border-zinc-200"
          >
  ...
  const completionPercent = Math.min(
    100,
    (stats.paid / Number(loan.principalAmount)) * 100,
  );
    ...
    <Container className="pb-10 space-y-6">
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              className="h-9 px-4 rounded-md border-zinc-200 bg-white text-zinc-600 font-bold gap-2 text-sm shadow-sm"
            >
      ...
      <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
        ...
        <div className="text-left md:text-right">
          ...
          <p className="text-sm font-medium text-zinc-700 max-w-sm">
            {loan.remarks || "No additional remarks"}
          </p>
        </div>
      </div>
        ...
        <div className="flex justify-between items-end">
          <div>
            ...
            <h4 className="text-2xl font-black text-zinc-900 mt-1">
              ...
              <span className="text-xs text-zinc-400 font-medium">SETTLED</span>
            </h4>
          </div>
        ...
        <div className="h-3 w-full bg-zinc-100 rounded-full overflow-hidden border border-zinc-200/50">
          ...
          >
            {completionPercent > 10 && (
              <div className="size-1.5 bg-white rounded-full shadow-sm animate-pulse" />
            )}
          </div>
        </div>
        ...
        <div className="p-5 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
          <div>
            ...
            <p className="text-xs text-zinc-500 font-medium">
              Tracking principal reductions and interest expenses.
            </p>
          </div>
          <div className="text-right">
