...
import { Landmark, CheckCircle2, TrendingDown, ArrowLeft, Banknote, FileDown } from "lucide-react";
...
export default function LoanDetailPage({ params }: { params: { id: string } }) {
    ...
    const detailColumns = useMemo(() => [
        ...
        },
        {
            header: "Interest",
            accessor: (row: ScheduleItem) => (
                <span className="text-amber-600 font-medium">{row.interest > 0 ? fmt(row.interest) : "—"}</span>
            )
        },
        {
            header: "Total",
